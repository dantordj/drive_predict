...
def predict_lgboost_k_fold(train_df,test_df,k):
    ...
    params = {'learning_rate': 0.01,'num_iterations':200, 'max_depth': -1, 'objective': 'binary', 'metric': 'auc','num_leaves': 50, 'min_data_in_leaf': 200, 'max_bin': 1000,'is_training_metric': False, 'seed': 99}
    ...
    for train, test in kf.split(train_df):
        ...
        # train
        gbm = lgb.train(params,lgb_train,num_boost_round=100,valid_sets=lgb_eval, verbose_eval=100,early_stopping_rounds=100)
        print('Start predicting...')
        # predict
        y_eval = gbm.predict(X_test, num_iteration=gbm.best_iteration)
